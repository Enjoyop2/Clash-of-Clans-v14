		...
		public override async void ProcessAsync()
		{
			if (Message.StartsWith('/'))
			{
				...
				switch (cmdType)
				{
					...
					case "/co":
						{
							GameObjectManager manager = Device.Player.Home.GameObjectManager;
			...
			{
				AllianceInfo info = Device.Player.Home.AllianceInfo;
				if (!info.HasAlliance) return;

				Logic.Clan.Alliance alliance = await Resources.Alliances.GetAllianceAsync(info.Id);
