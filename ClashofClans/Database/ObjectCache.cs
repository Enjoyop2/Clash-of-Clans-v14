	...
	public class ObjectCache
	{
		...
		public void CachePlayer(Player player)
		{
			try
			{
				...
				string json = home.GameObjectManager.Save();

				ICacheEntry playerEntry = _playerCache.CreateEntry(home.Id);
				playerEntry.Value = home;
				_playerCache.Set(home.Id, playerEntry, _expirationTimeSpan);

				ICacheEntry objectEntry = _objectCache.CreateEntry(home.Id);
		...
		public void CacheAlliance(Alliance alliance)
		{
			try
			{
				ICacheEntry allianceEntry = _allianceCache.CreateEntry(alliance.Id);
