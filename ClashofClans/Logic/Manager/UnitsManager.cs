...
namespace ClashofClans.Logic.Manager
{
	public class UnitsManager
	{
		...
		{
			int index = Troops.FindIndex(troop => troop.Id == id);
		...
		{
			int index = Spells.FindIndex(spell => spell.Id == id);
		...
		{
			int index = SiegeMachines.FindIndex(machine => machine.Id == id);
		...
		{
			int index = Pets.FindIndex(pet => pet.Id == id);
		...
		{
			int index = Pets.FindIndex(pet => pet.Id == petId);
		...
		{
			int index = SiegeMachines.FindIndex(spell => spell.Id == id);
		...
		{
			int index = TroopsV2.FindIndex(troop => troop.Id == id);
		...
		{
			int troopIndex = Troops.FindIndex(troop => troop.Id == id);
			int machineIndex = SiegeMachines.FindIndex(troop => troop.Id == id);
		...
		{
			int index = Spells.FindIndex(troop => troop.Id == id);
