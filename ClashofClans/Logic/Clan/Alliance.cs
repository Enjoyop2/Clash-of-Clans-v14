...
namespace ClashofClans.Logic.Clan
{
	public partial class Alliance
	{
		...
		public void AllianceHeaderEntry(ByteBuffer packet)
		{
			...
			foreach (int label in ClanLabels)
			{
		...
		{
			int index = Members.FindIndex(x => x.Id == id);
		...
		public async void AddEntry(AllianceStreamEntry entry)
		{
			lock (Stream)
			{
				...
				int max = Stream.Count == 0 ? 1 : Stream.Max(x => x.Id);
				entry.Id = max == int.MaxValue ? 1 : max + 1; // If we ever reach that value... but who knows...

				Stream.Add(entry);
			}

			foreach (AllianceMember member in Members.Where(m => m.IsOnline).ToList())
			{
				Player player = await member.GetPlayerAsync(true);
